...
\item calculate statistical properties of the variables, such as running averages and standard deviations, correlation functions of pairs of variables, and multidimensional histograms: this can be done either at run-time without the need to save very large trajectory files, or after a simulation has been completed using VMD and the \formatkey{cv} command\cvnamdugonly{ or NAMD and the \formatkey{coorfile read} command as illustrated in \ref{section:sample}}.
...
or by using the \formatkey{group2ndx} LAMMPS command.}
...
\formatkey{units} keyword to the general parameters, outside of \formatkey{colvar \{\}} blocks (\ref{sec:units}).
...
Atom selections should be inserted within an atom group block, within a component block (such as \formatkey{distance}).
...
Special fields (O, B, and user) may be used, as well as atom positions (e.g. \formatkey{z > 0}).
...
\emph{Set scaling factor} rescales gradients by the specified factor, divided by the colvar's \formatkey{width} parameter (1 by default, see~\ref{sec:colvar_grid_params}).
This use of \formatkey{width} makes it easier to compare the gradients of collective variables that are not commensurate.
...
\key
  ...
  However, the \formatkey{units} keyword will switch to a different unit system than the
  current one if no colvars were defined before reading the current configuration.
  If colvars are already defined, \formatkey{units} will refuse changing the unit system to avoid making the definition of those variables erroneous in the new system of units.
  If needed this precaution can be overridden using the \formatkey{cv units} scripting command
...
To enable a Colvars-based calculation, the \formatkey{colvars on} command must be added to the NAMD script.
Two optional commands, \formatkey{colvarsConfig} and \formatkey{colvarsInput} can be used to define the module's configuration or continue a previous simulation.
Because these are static parameters, it is typically more convenient to use the \formatkey{cv} command in the rest of the NAMD script.
...
\item %
  \key
    ...
    This file can also be provided by the Tcl command \formatkey{cv configfile}.
    Alternatively, the contents of the file (as opposed to the file itself) can be given as a string argument to the command \formatkey{cv config}.
  }
...
\item %
  \key
    ...
    If the input file is meant to be loaded within a Tcl script section, the \formatkey{cv load} command may be used instead.
  }
...
\item %
  \key
    {input}{%
    Keyword of the \formatkey{fix colvars} command}{%
...
\item %
  \keydef
    ...
    This is only meaningful when the \formatkey{extendedLangevinDamping} keyword is used (see \ref{sec:colvar_extended}).}
...
\cvscriptonly{
\cvsubsec{Using the \formatkey{cv} command to control the Colvars module}{sec:cv_scripting}
\cvvmdugonly{\index{Colvars!\texttt{cv} command}}

At any moment after the first initialization of the Colvars module, several options can be read or modified by the Tcl command \formatkey{cv}, with the following syntax:\\
\noindent\texttt{cv~$<$subcommand$>$ [args ...]}\\
\cvvmdonly{The \formatkey{cv} command is used by the Dashboard graphical interface,(\ref{sec:dashboard}), but can be also used in scripts or interactively from the command-line terminal (for example, in remote terminal sessions) or in the Tk Console.}
The most frequent uses of the \formatkey{cv} command are discussed here.
For a complete list of all sub-commands of \formatkey{cv}, see section~\ref{sec:cvscript_tcl}.
...
\noindent{}will attach the Colvars module onto the molecule currently holding the ``top'' status (alternatively, you can refer to a molecule by its numeric ID in lieu of \formatkey{top}).
All following invocations of the \formatkey{cv} command will continue operating on the same molecule, regardless of whether other molecules are loaded, or which one has the ``top'' status.
The \formatkey{cv molid} command without argument will return the molid currently associated with Colvars.
}

\cvnamdonly{
  If the NAMD configuration parameter \formatkey{colvars} is on, the \formatkey{cv} command can be used anywhere in the NAMD script, and will be invoked as soon as NAMD begins processing Tcl commands.
}
...
\formatkey{cv version} can be used to get the Colvars version for this use:\\
{\noindent\ttfamily
...
After a configuration is fully defined, \formatkey{cv load} may be used to load a state file from a previous simulation that contains e.g.{} data from history-dependent biases), to either continue that simulation or analyze its results:\\
...
\cvnamdonly{\noindent{}The latter is much more convenient in combination with the NAMD \formatkey{reinitatoms} command, for example:\\
{\noindent\ttfamily
  reinitatoms $<$oldjob$>$\\
  cv load $<$oldjob$>$\\
}
The step number contained by the loaded file will be used internally by Colvars to control time-dependent biases, unless \formatkey{firstTimestep} is issued, in which case that value will be used.}

\cvnamdonly{When the system's topology is changed during simulation via the \formatkey{structure} command (e.g.{} in constant-pH simulations), it is generally best to reset and re-initalize the module from scratch before loading the corresponding snapshot:\\
{\noindent\ttfamily
...
\formatkey{cv save}, analogous to \formatkey{cv load}, saves all restart information to a state file.
...
Because not only a state file (used to continue simulations) but also other data files (used to analyze the trajectory) are written, it is generally clearer to use \formatkey{cv save} with a prefix rather than a file name:\\
...
After one or more collective variables are defined, they can be accessed via \formatkey{cv colvar [args ...]}.
For example, to recompute the collective variable \formatkey{xi} the following command can be used:\\
...
However, when analyzing an existing trajectory a call to \formatkey{update} is generally required.

While in all typical cases all configuration of the variables is done with \formatkey{cv config} or \formatkey{cv configfile}, a limited set of changes can be enacted at runtime using \formatkey{cv colvar $<$name$>$ modifycvcs [args ...]}.
...
For example, a variable \formatkey{DeltaZ} made of a single \formatkey{distanceZ} CVC can be made periodic with a period equal to the unit cell dimension along the Z-axis:\\
\noindent\texttt{cv colvar DeltaZ modifycvcs "period \$Lz"}\\
\cvnamdonly{\noindent{}where \formatkey{\$Lz} is obtained outside Colvars.\\}
\cvvmdonly{\noindent{}where \formatkey{\$Lz} may be obtained for example as:\\
\noindent\texttt{set Lz [molinfo top get c]}.\\}
\noindent{}This option is currently limited to changing the values of \refkey{componentCoeff}{sec:cvc_superp} and \refkey{componentExp}{sec:cvc_superp} (e.g.{} to update the polynomial superposition parameters on the fly), of \refkey{period}{sec:cvc_periodic} and \refkey{wrapAround}{sec:cvc_periodic}, and of the \formatkey{forceNoPBC} option for all components that support it.
...
\noindent{}where \formatkey{$<$flags$>$} is a list of 0/1 values, one per component.
...
\textbf{Important:} None of the changes enacted by \formatkey{modifycvcs} or \formatkey{cvcflags} will be saved to state files, and will be lost when restarting a simulation, deleting the corresponding collective variable, or resetting the module with \cvvmdonly{\formatkey{cv delete} or} \formatkey{cv reset}.

\cvsubsubsec{Applying and analyzing forces on collective variables}{sec:cv_command_colvar_force}

As soon as a collective variable is up to date (during a MD run or after its \formatkey{update} method has been called), forces can be applied to it, e.g.{} as part of a custom restraint implemented by \refkey{scriptedColvarForces}{Colvars-global|scriptedColvarForces}:\\
\noindent\texttt{cv colvar xi addforce \$force}\\
\noindent{}where \formatkey{\$force} is a scalar or a vector (depending on the type of variable \formatkey{xi}) and is defined by the user's function.
...
Until then, the total force applied to \formatkey{xi} from all biases can be retrieved by:\\
...
To obtain the total force projected on the variable \formatkey{xi}:\\
...
For example, given the most current value of the variable \formatkey{xi}, an already-defined restraint on it named \formatkey{harmonic\_xi} can be updated as:\\
...
Calling \formatkey{update} for a bias is most useful for just-defined biases or when changing their configuration.
When \formatkey{update} is called e.g.{} as part of the function invoked by \refkey{scriptedColvarForces}{Colvars-global|scriptedColvarForces}, it is executed before any biasing forces are applied to the variables, thus allowing to modify them.
This use of \formatkey{update} is often used e.g.{} in the definition of custom bias-exchange algorithms as part of the \MDENGINE{} script.
...
A corresponding \formatkey{save} function is also available:\\
...
\cvnamdonly{All the parameters defining variables and their biasing or analysis algorithms are read from the file specified by the configuration option \formatkey{colvarsConfig}, or by the Tcl commands \formatkey{cv config} and \formatkey{cv configfile}.
\emph{None of the keywords described in the remainder of this manual are recognized directly in the NAMD configuration file, unless as arguments of \formatkey{cv config}.}}
\cvvmdonly{The Colvars configuration is usually read using the commands \formatkey{cv configfile} (with a filename as argument) or \formatkey{cv config} (with the configuration as a string argument).}
Each configuration line follows the format \formatkey{keyword value}, where the keyword and its value are separated by any white space.
The following rules apply:

\begin{itemize}

\item keywords are case-insensitive (\formatkey{upperBoundary} is the same as \formatkey{upperboundary} and \formatkey{UPPERBOUNDARY}): their string values are however case-sensitive (e.g.~file names);

\item a long value, or a list of multiple values, can be distributed across multiple lines by using curly braces, \formatkey{\{} and \formatkey{\}}: the opening brace \formatkey{\{} must occur on the same line as the keyword, following a space character or other white space; the closing brace \formatkey{\}} can be at any position after that; any keywords following the closing brace on the same line are not valid (they should appear instead on a different line);
...
\cvnamdonly{%
\item the \formatkey{=} sign between a keyword and its value, deprecated in the NAMD main configuration file, is not allowed;

\item Tcl syntax is generally not available, but it is possible to use Tcl variables or bracket expansion of commands within a configuration string, when this is passed via the command \formatkey{cv config \ldots}; this is particularly useful when combined with parameter introspection\cvnamdugonly{ (see \ref{section:tclscripting})}, e.g.{} \formatkey{cv config "colvarsTrajFrequency [DCDFreq]"};
}

\cvvmdonly{%
\item Tcl syntax is generally not available, but it is possible to use Tcl variables or bracket expansion of commands within a configuration string, when this is passed via the command \formatkey{cv config \ldots}: for example, it is possible to convert the atom selection \$\emph{sel} into an atom group (see \ref{sec:colvar_atom_groups_sel}) using \formatkey{cv config "atomNumbers \{ [\$sel get serial] \}"};}

\item if a keyword requiring a boolean value (\formatkey{yes|on|true} or \formatkey{no|off|false}) is provided without an explicit value, it defaults to \formatkey{yes|on|true}; for example, \formatkey{outputAppliedForce} may be used as shorthand for \formatkey{outputAppliedForce on};

\item the hash character \formatkey{\#} indicates a comment: all text in the same line following this character will be ignored.
